...
root.bind('<Return>',
          add_task)  # bind return key to add_task so that enter key can add task without clicking the button
...
def delete_task():
    ...
    if task in tasks:
        confirm_del = messagebox.askyesno("Confirm Deletion",
                                          "Are you sure you want to delete task:   ** {} ** ?".format(task))
...
lbl_title.config(font=("Courier", 44), fg="blue")
lbl_title.grid(row=0, column=1)

lbl_display = tkinter.Label(root, text="Random Task", bg="white")
lbl_display.config(font=("Courier", 34), fg="grey")
lbl_display.grid(row=0, column=3)

txt_input = tkinter.Entry(root, width=35, font=("Courier", 20))
txt_input.grid(row=1, column=1)

btn_add_task = tkinter.Button(root, text="Add Task", fg="green", bg="white", command=add_task, height=2, width=30)
...
btn_num_tasks = tkinter.Button(root, text="Number of Tasks", fg="green", bg="white", command=num_tasks, height=2,
                               width=30)
btn_num_tasks.grid(row=3, column=0)

btn_delete_task = tkinter.Button(root, text="Delete Task", fg="green", bg="white", command=delete_task, height=2,
                                 width=30)
btn_delete_task.grid(row=4, column=0)

btn_delete_all = tkinter.Button(root, text="Delete All", fg="green", bg="white", command=delete_all, height=2, width=30)
...
btn_rand_task = tkinter.Button(root, text="Random Task", fg="green", bg="white", command=rand_task, height=2, width=30)
btn_rand_task.grid(row=8, column=0)

btn_quit_program = tkinter.Button(root, text="Exit", fg="green", bg="white", command=exit, height=2, width=30)
btn_quit_program.grid(row=9, column=0)

lb_tasks = tkinter.Listbox(root)
lb_tasks.config(font=("Courier", 32), fg="pink")
